...
export default function ChatIA({ claseId }) {
  ...
  const handlePregunta = async (e) => {
    ...
    setHistorial((prev) => [
      ...prev,
      { tipo: "usuario", texto: preguntaActual },
    ]);
    setPregunta("");
    ...
    try {
      ...
      setHistorial((prev) => [...prev, { tipo: "ia", texto: respuestaIA }]);
      ...
      if (insertError) {
        // No mostramos este error al usuario, pero es vital registrarlo para depuración.
        console.error("Error al guardar la pregunta en la BD:", insertError);
      }
    } catch (error) {
      ...
      setHistorial((prev) => [
        ...prev,
        {
          tipo: "ia",
          texto:
            "Lo siento, ha ocurrido un error al procesar tu pregunta. Por favor, inténtalo de nuevo.",
        },
      ]);
    } finally {
      setIsLoading(false);
    ...
    <div className="relative group mt-8">
      ...
      <div className="relative p-6 bg-white rounded-2xl shadow-lg border border-slate-200">
        ...
        <div className="mb-4 space-y-4 max-h-80 overflow-y-auto pr-4">
          {historial.map((item, index) => (
            <div
              key={index}
              className={`flex ${
                item.tipo === "usuario" ? "justify-end" : "justify-start"
              }`}
            >
              <div
                className={`max-w-prose px-4 py-3 rounded-xl shadow-sm ${
                  item.tipo === "usuario"
        ...
        <p className="text-xs text-slate-500 mt-2">
          La IA responderá basándose únicamente en el contenido de esta clase.
        </p>
      </div>
    </div>
  );
